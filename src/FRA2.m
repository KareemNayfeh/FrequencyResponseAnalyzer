...
try    
    ...
    for kk = 1:length(freqList)
        ...
        minIntUs = 10;
        optimalBlockUs = 1e6/freq(kk)/numSamplesPerWave
        
        blockIntUs = min(minIntUs, optimalBlockUs);
        ...
        numSamples = min(numSamples, maxBlockSamples);
        numSamples = max(numSamples, 64);
        ...
        % 
        % magA = sqrt(AA(kk)^2+BA(kk)^2);
        % magB = sqrt(AB(kk)^2+BB(kk)^2);
        % 
        % phaseA = (atan2(BA(kk), AA(kk)));
        % phaseB = (atan2(BB(kk), AB(kk)));
        % 
        % Ac(kk) = AA(kk)+i*BA(kk);
        % Bc(kk) = AB(kk)+i*BB(kk);
        % 
